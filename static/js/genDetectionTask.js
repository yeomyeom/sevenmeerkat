...
3) 기능
    (1) canvas에 작업자가 bounding box를 그리면 그에 대한 좌표를 제출 버튼 클릭시 서버로 전송
    (2) 이전에 bounding box가 그려져 있기 때문에 좌표값이 서버에서 전달된 경우라면 canvas에
        bounding box를 그림
*/
...
{
    /*
    NOTE: 아래에 사용된 jsonData 는 typeSelectionControl.js 에서 불러온 jsonData 이므로 
    위에서 Declaration을 하지 않았음
    */
    valueInit(jsonData.width, jsonData.height, jsonData.label); 
    drawAll();  // <= drawAll() function이 bounding box 좌표를 바탕으로 canvas에 bounding box를 그려주는 역할을 함
...
function valueInit(inputW, inputH, inputArray){
    /* 
    NOTE: 서버에서 받은 값이 있다면 (jsonData라는 object의 label key로 들어오는 것)
    이를 위에서 선언된 Rect로 assign 해주는 역할
    */
    N = inputArray.length;
    W = inputW;
    H = inputH;
    for(let i = 0; i < N ; i++ ){
        Rect[i] = {x:[], y:[]};
        Rect[i].x[0] = inputArray[i].x_min;
        Rect[i].y[0] = inputArray[i].y_min;
        Rect[i].x[1] = inputArray[i].x_max;
        Rect[i].y[1] = inputArray[i].y_max;
    } }
...
function returnRectToJSON() {
    ...
    for(let i = 0; i < N ; i++){
        ...
        jsonObj.x_min = parseInt(Rect[i].x[0]);
        jsonObj.y_min = parseInt(Rect[i].y[0]);
        jsonObj.x_max = parseInt(Rect[i].x[1]);
        jsonObj.y_max = parseInt(Rect[i].y[1]);
        outputRect.push(jsonObj);
    }

    let lastData = new Object();
    lastData.label = outputRect;
    lastData.file_name = jsonData.file_name;
...
/* 
NOTE: 이제는 삭제한 기능이나 나중에 추가될 수 있음
*/

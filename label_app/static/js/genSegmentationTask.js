...
// window.onload = loadImage(jsonData);
...
function valueInit(inputW, inputH, inputArray) {
    ...
    for (let i = 0; i < N; i++) {
        ...
        for (let j = 0; j < PolygonCount[i]; j++)
            Polygon[i][j] = {
                x: inputArray[i][j].x, y: inputArray[i][j].y
            };
    }
}
function clicking(e) {
...
function returnPolyToJSON() {
    ...
    let iPoly = [];
    let outputPoly = new Array();
    for (let i = 0; i < N; i++) {
        iPoly[i] = {
            x: [], y: []
        };
        let midPoly = new Array();
        for (let j = 0; j < PolygonCount[i]; j++) {
            iPoly[i].x[j] = parseInt(Polygon[i][j].x);
            iPoly[i].y[j] = parseInt(Polygon[i][j].y);

            var jsonObj = new Object();
            jsonObj.x = parseInt(Polygon[i][j].x);
            jsonObj.y = parseInt(Polygon[i][j].y);
            midPoly.push(jsonObj);
        }
        outputPoly.push(midPoly);

    }

    let lastData = new Object();
    lastData.label = outputPoly;
